...
class LearnableBias(nn.Module):
    def __init__(self, channel):
        ...
        self.k_1 = nn.Parameter(torch.ones(1, channel,1,1), requires_grad = True)
        self.k_2 = nn.Parameter(torch.zeros(1, channel,1,1), requires_grad = True)
        
        self.k_3 = nn.Parameter(torch.ones(1, channel,1,1), requires_grad = True)
        self.k_4 = nn.Parameter(torch.zeros(1, channel,1,1), requires_grad = True)
        
        self.kb = nn.Parameter(torch.ones(1, channel,1,1), requires_grad = True)
    ...
    def forward(self,x, UB_ops):
        ...
        if UB_ops[4] == 10:
            p_1 = self.sb.expand_as(x)*x_1 + (1-self.sb).expand_as(x)*x_2
        else:
            p_1 = Binary_op(x_1,x_2, UB_ops[4])

        
        
        if UB_ops[2] == 19:
            h_1 = self.k_1.expand_as(p_1)*p_1
        elif UB_ops[2] == 20:
            h_1 = p_1 + self.k_2.expand_as(p_1)
        elif UB_ops[2] == 21:
            h_1 = self.k_1.expand_as(p_1)
        else:
            h_1 = Unary_op(p_1, UB_ops[2])

        if UB_ops[3] == 19:
            h_2 = self.k_3.expand_as(x)*x
        elif UB_ops[3] == 20:
            h_2 = x + self.k_4.expand_as(x)
        elif UB_ops[3] == 21:
            h_2 = self.k_3.expand_as(x)
        else:
            h_2 = Unary_op(x, UB_ops[3])
        
        if UB_ops[5] == 10:
            out = self.kb.expand_as(x)*h_1 + (1-self.kb).expand_as(x)*h_2
        else:
            out = Binary_op(h_1,h_2, UB_ops[5])
